...
for iTask = 1:length(task)
    ...
    for iROI = 1:length(ROI)
        ...
        saveas(gcf,strcat(curTask, thisROIName, 'IGERP', num2str(bl(1)), date()),'epsc');
        saveas(gcf,strcat(curTask, thisROIName, 'IGERP', num2str(bl(1)), date()),'png');
        cnt = cnt + 1;
        close all;
    ...
    for iROI = 1:length(ROI)
        ...
        saveas(gcf,strcat(curTask, thisROIName, 'OGERP', num2str(bl(1)), date()),'epsc');
        saveas(gcf,strcat(curTask, thisROIName, 'OGERP', num2str(bl(1)), date()),'png');
        cnt = cnt + 1;
        close all;
    end
