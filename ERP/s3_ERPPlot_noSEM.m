...
for iTask = 1:length(task)
    ...
    for iROI = 1:length(ROI)
        ...
        saveas(gcf,strcat('noSE_', curTask, thisROIName, 'IGERP', date()),'epsc');
        saveas(gcf,strcat('noSE_', curTask, thisROIName, 'IGERP', date()),'png');
        cnt = cnt + 1;
        close all;
    ...
    for iROI = 1:length(ROI)
        ...
        saveas(gcf,strcat('noSE_',curTask, thisROIName, 'OGERP', date()),'epsc');
        saveas(gcf,strcat('noSE_',curTask, thisROIName, 'OGERP', date()),'png');
        cnt = cnt + 1;
        close all;
    end
