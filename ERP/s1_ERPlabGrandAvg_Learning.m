...
for iTask = 1:length(task)
    ...
    ERPsetList = [curTask 'GrandAvgAll_epoch' num2str(bl(1)), '.txt']; % create a txt file to save the subject level ERP sets to be involved in grand averge
    ...
    for iSubject = 1:length(subNameD)
        ...
        [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 0,'savenew',...
            [outputSubjectFolder filesep subName '_' curTask '_rmbl', num2str(bl(1))],'gui','off');
        
        ALLEEG = []; EEG = []; CURRENTSET = [];
        
        EEG = pop_loadset('filename',[subName '_' curTask '_rmbl',num2str(bl(1)) '.set'],'filepath',outputSubjectFolder);
        [ALLEEG, EEG, CURRENTSET] = eeg_store( ALLEEG, EEG, 0 );
        ...
        ERP = pop_savemyerp(ERP, 'erpname',...
            [subName '_' curTask '_ERP_Avg',num2str(bl(1))], 'filename', [subName '_' curTask '_ERPAvg' num2str(bl(1)) '.erp'], 'filepath', outputSubjectFolder, 'Warning','off');
        fprintf(fid,[outputSubjectFolder '/' subName '_' curTask '_ERPAvg' num2str(bl(1)) '.erp\n']);
    ...
    ERP = pop_gaverager( [outputParentFolder '/' curTask 'ERPlabGrandAvg/' ERPsetList] , 'SEM', 'on',...
        'Weighted', 'on' );
    ERP = pop_savemyerp(ERP, 'erpname', 'GrandAvgAll', 'filename', [curTask '_GrandAvg_ManuRej' num2str(bl(1)), '.erp'], 'filepath',...
