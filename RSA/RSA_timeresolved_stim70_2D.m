...
path_in  = data_folder;
savName = strcat ('/', 'Results_RSA2D_', task ,'_SR',num2str(sr),'WIN',num2str(win),'SLIDE',num2str(slide), '_', ztrans, '_'); 
path_out = fullfile(saveLocation, 'Results',strcat('RSA2D_', task, '_SR',num2str(sr),'WIN',num2str(win),'SLIDE',num2str(slide),'_', ztrans));
mkdir(path_out)
...
for n=1:numel(all_subs)
    load(fullfile(path_out, strcat(savName, all_subs{n},'.mat')))
    
    % option 1 using all trials:
    trial_corr_all{n}=corr_trials.corr_exp_mor_trial;
    trialinfo_all{n}=corr_trials.trialinfo;
    
    % option 2 using averaged trials:
    trial_corr_all{n}=corr_trials.corr_exp_mor_stim;
    trialinfo_all{n} =struct2table(corr_trials.stiminfo);
