...
path_out = fullfile(saveLocation, 'Stats',strcat('RSA2D_DiffIOG_', task, '_SR',num2str(sr),'WIN',num2str(win),'SLIDE',num2str(slide),'_', ztrans));
mkdir(path_out)
...
data_high   = data_in_high - data_out_high;
data_low    = data_in_low  - data_out_low;
data_con    = data_in_con  - data_out_con;
data_hl     = data_in_hl   - data_out_hl;
data_hc     = data_in_hc   - data_out_hc;
data_cl     = data_in_cl   - data_out_cl;

data_comp_all = {
    data_in_high, data_in_low, data_in_con,...
    data_out_high, data_out_low, data_out_con,...
    data_in_hl, data_in_hc, data_in_cl,...
    data_out_hl, data_out_hc, data_out_cl,...
    data_high, data_low, data_con,...
    data_hl, data_hc, data_cl};
contrast_type = {
    'ingroup-higher', 'ingroup-lower', 'ingroup-consistent',...
    'outgroup-higher', 'outgroup-lower', 'outgroup-consistent',...
    'ingroup-higher-lower', 'ingroup-higher-consistent', 'ingroup-consistent-lower',...
    'outgroup-higher-lower', 'outgroup-higher-consistent', 'outgroup-consistent-lower',...
    'higher', 'lower', 'consistent',...
...
pre_data_in_hl = pre_data_in_high - pre_data_in_low;
pre_data_in_hc = pre_data_in_high - pre_data_in_con;
pre_data_in_cl = pre_data_in_con  - pre_data_in_low;
pre_data_out_hl = pre_data_out_high - pre_data_out_low;
pre_data_out_hc = pre_data_out_high - pre_data_out_con;
pre_data_out_cl = pre_data_out_con  - pre_data_out_low;

data_comp_pre = {
    pre_data_in_high,pre_data_in_low,pre_data_in_con,...
    pre_data_out_high,pre_data_out_low,pre_data_out_con,...
    pre_data_in_hl, pre_data_in_hc, pre_data_in_cl,...
    pre_data_out_hl, pre_data_out_hc, pre_data_out_cl};
...
post_data_in_hl = post_data_in_high - post_data_in_low;
post_data_in_hc = post_data_in_high - post_data_in_con;
post_data_in_cl = post_data_in_con  - post_data_in_low;
post_data_out_hl = post_data_out_high - post_data_out_low;
post_data_out_hc = post_data_out_high - post_data_out_con;
post_data_out_cl = post_data_out_con  - post_data_out_low;

data_comp_post = {
    post_data_in_high,post_data_in_low,post_data_in_con,...
    post_data_out_high,post_data_out_low,post_data_out_con,...
    post_data_in_hl, post_data_in_hc, post_data_in_cl,...
    post_data_out_hl, post_data_out_hc, post_data_out_cl};

% statistical analysis

Stats_Results = struct;

for i_cont = 13:length(contrast_type)
