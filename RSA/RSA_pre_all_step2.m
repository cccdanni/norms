...
cd( beha_folder);
behadata = readmatrix ('PreLearningExplicitBehaData2021-02-23.csv');
...
this_rsa_folder = fullfile(rsa_folder, 'all_trials_pre_sr100windowfullslidenone');
cd( this_rsa_folder );
...
for sub_nr = 1:numel(subs)
    ...
    cd(this_rsa_folder);
    load ([thissub 'pre_alltrials_full.mat']); % corr_res, erpdata, erpdata3
    
    corr_res = 1-corr_res; % simialrity to RDM
    
    thisERPfolder = fullfile (erp_folder, thissub);
    cd(thisERPfolder);
    EEG = pop_loadset ([thissub '_preLearningImplicit_EpochArtRej_FalseRespRej.set']);
    ...
    cd(this_rsa_folder);
    save([thissub 'pre_alltrials_full_trans.mat'], 'neural_rdm', 'thisbinlist',...
        'conceptual_rdm_conflict', 'conceptual_rdm_inconflict', 'behavior_rdm');
    clear thissub corr_res corr_res_update thisbinlist newseq;
end

save('pre_alltrials_full_trans.mat', 'allsubject_behardm', 'allsubject_neuralrdm',....
