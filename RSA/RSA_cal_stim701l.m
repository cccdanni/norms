...
function RSA_timeresolved_stim701l(subs, work_dir) 
...
path_in  = data_folder;
savName = strcat ('/', 'Results_RSA1l_', task ,'_SR',num2str(sr),'WIN',num2str(win),'SLIDE',num2str(slide), '_', ztrans, '_'); 
path_out = fullfile(saveLocation, 'Results',strcat('RSA1l_', task, '_SR',num2str(sr),'WIN',num2str(win),'SLIDE',num2str(slide),'_', ztrans));
mkdir(path_out)
...
for sub_nr = 1:numel(subs)
    ...
    % ingroup / outgroup lower is dissimilar to morphed faces (dissimilar = 1)
    ...
    for i = 1: n_cond*10
        for j = 1: n_cond*10
            ...
            elseif (ismember(j, [11:20])) & (ismember(i, [61:70]))
                conceptual_rdm_inconflict (i, j ) = 1;            
            elseif (ceil(i/10) == ceil(j/10))
    ...
    % ingroup lower is dissimilar to morphed faces (dissimilar = 1)
    % others is less dissimilar to morphed faces (dissimilar = 1)
    ...
    for i = 1: n_cond*10
        for j = 1: n_cond*10
            ...
            elseif (ismember(j, [41:50])) & (ismember(i, [61:70]))
                conceptual_rdm_outconflict (i, j ) = 1;               
            elseif (ceil(i/10) == ceil(j/10))
    ...
    % outgroup lower is dissimilar to morphed faces (dissimilar = 1)
    % others is less dissimilar to morphed faces (dissimilar = 1)
