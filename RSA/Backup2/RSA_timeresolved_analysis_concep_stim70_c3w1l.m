...
path_in  = data_folder;
savName = strcat ('Results_RSAc3w1l_concepImplicit_SR',num2str(sr),'WIN',num2str(win),'SLIDE',num2str(slide), '_', ztrans, '_'); 

path_pre  = fullfile(saveLocation, 'Results',strcat('RSAc3w1l_preLearningImplicit_SR',num2str(sr),'WIN',num2str(win),'SLIDE',num2str(slide),'_', ztrans));
path_post = fullfile(saveLocation, 'Results',strcat('RSAc3w1l_postLearningImplicit_SR',num2str(sr),'WIN',num2str(win),'SLIDE',num2str(slide),'_', ztrans));
path_diff = fullfile(saveLocation, 'Stats',strcat('RSAc3w1l_concepImplicit_SR',num2str(sr),'WIN',num2str(win),'SLIDE',num2str(slide),'_', ztrans));
if ~exist (path_diff)
    mkdir(path_diff)
end
path_out = path_diff;
...
RDM_pre = load ( strcat (path_pre, '/', 'Results_RSAc3w1l_preLearningImplicit_SR',num2str(sr),'WIN',num2str(win),'SLIDE',num2str(slide), '_', ztrans, '_trans.mat') );
RDM_post = load ( strcat (path_post, '/', 'Results_RSAc3w1l_postLearningImplicit_SR',num2str(sr),'WIN',num2str(win),'SLIDE',num2str(slide), '_', ztrans, '_trans.mat') );
...
for iContrast = 1 : length (contrast_names)
    ...
    if strcmp (perm_test, 'yes')
        ...
        %p_threshold=p_threshold/2; % two sided test; DC: I edited here, so that we could compare the p_threshold
...
%% Ststistical Testing
Results_Stats = struct; 
contrast_names = {'neural_conp_all', 'neural_conp_in', 'neural_conp_out',...
    'neural_beha', 'in_out'};
all_R = {allsubject_neuralconp_all_R,...
    allsubject_neuralconp_in_R,... 
    allsubject_neuralconp_out_R,... 
