...
% Update date: 2022/1/11
% 70 stimuli (experimental faces & morphed faces, no control faces)
% control facial similarity 
...
script_folder = fullfile (work_dir, 'MyScripts');
...
control_folder = fullfile (work_dir, 'StimSim');
face_info = readtable (fullfile (control_folder, 'allFaceStimulus_u.xlsx'));
face_dts  = readtable (fullfile (control_folder, 'face_recognition.csv'));
for i = 1:size(face_dts,1)
    img1 = table2cell (face_dts(i, "img1"));
    img2 = table2cell (face_dts(i, "img2"));
    id1 = face_info(strcmpi (face_info.photoName, img1),'FigureID');
    id2 = face_info(strcmpi (face_info.photoName, img2),'FigureID');
    face_dts(i, "figureID_1") = id1;
    face_dts(i, "figureID_2") = id2;
end
clear face_info i img1 img2 id1 id2;
...
path_in  = data_folder;
savName = strcat ('Results_RSA_concepImplicit_SR',num2str(sr),'WIN',num2str(win),'SLIDE',num2str(slide), '_', ztrans, '_c_'); 
...
path_diff = fullfile(saveLocation, 'Stats',strcat('RSA_concepImplicit_SR',num2str(sr),'WIN',num2str(win),'SLIDE',num2str(slide),'_', ztrans, '_c'));
if ~exist (path_diff)
    mkdir(path_diff)
end
path_out = path_diff;
...
facerdm_allsub = nan (nCond*10, nCond*10, nBins, nSubs);

for nSub = 1: nSubs
    
    %% re-construct the facial-dissimilarity matrix
    thissub = all_subs{nSub};
    sel_sub_folder = fullfile( data_folder, thissub );
    EEG = pop_loadset('filename',char(strcat( thissub, '_postLearningImplicit_EpochArtRej_FalseRespRej.set')),...
        'filepath',fullfile(sel_sub_folder,'/'));
    ...
    thisbinlist = zeros(nCond, 10);
    for i = 1:nCond % nCond bins
        x = ceil((min(thiscode(thisbini == bin_list(i))))/10);
        x1 = ((x-1)*10+1):x*10;
        thisbinlist(i,:) = x1;
    end % check each bin includes which figures
    newseq = reshape(thisbinlist',1,nCond*10);
    
    facedts_mat = nan (nCond*10, nCond*10);
    for i_fig = 1: nCond*10
        for j_fig = 1:nCond*10
           facedts_mat(i_fig, j_fig) = table2array (face_dts (eq(face_dts.figureID_1,  newseq(i_fig)) & eq(face_dts.figureID_2,  newseq(j_fig)), 'distance') );
        end
    end
    
    %% correlation between neural-RDM and conceptual RDM while controlling the facial distance

    for nBin = 1: nBins

        behardm     = RDM_pre.allsubject_behardm_allbins (:, :, nBin, nSub);
        conprdm_all = RDM_pre.allsubject_coceprdm_conflict_allbins (:, :, nBin, nSub);
        conprdm_in  = RDM_pre.allsubject_coceprdm_inconflict_allbins (:, :, nBin, nSub);
        conprdm_out = RDM_pre.allsubject_coceprdm_outconflict_allbins (:, :, nBin, nSub);
        ...
        facerdm_VECTOR = matrix2vector (facerdm, "lower");
        %[neuralconp_all_corr, neuralconp_all_corr_p]  = corr(neuralrdm_VECTOR', conprdm_VECTOR', 'rows','complete', 'type','Spearman');
        [neuralconp_all_corr, neuralconp_all_corr_p]  = partialcorr (neuralrdm_VECTOR', conprdm_VECTOR', facerdm_VECTOR', 'rows','complete', 'type','Spearman'); % partial correlation, by controlling facial stimuli distance
        ...
        neuralrdm_VECTOR = matrix2vector (neuralrdm, "lower");
        facerdm_VECTOR = matrix2vector (facerdm, "lower");
        %[neuralconp_in_corr, neuralconp_in_corr_p]  = corr(neuralrdm_VECTOR', conprdm_VECTOR', 'rows','complete', 'type','Spearman');
        [neuralconp_in_corr, neuralconp_in_corr_p]  = partialcorr(neuralrdm_VECTOR', conprdm_VECTOR', facerdm_VECTOR', 'rows','complete', 'type','Spearman');
        ...
        neuralrdm_VECTOR = matrix2vector (neuralrdm, "lower");
        facerdm_VECTOR = matrix2vector (facerdm, "lower");
        %[neuralconp_out_corr, neuralconp_out_corr_p]  = corr(neuralrdm_VECTOR', conprdm_VECTOR', 'rows','complete', 'type','Spearman');
        [neuralconp_out_corr, neuralconp_out_corr_p]  = partialcorr(neuralrdm_VECTOR', conprdm_VECTOR', facerdm_VECTOR', 'rows','complete', 'type','Spearman');
        allsubject_neuralconp_out_R (nSub, nBin) = neuralconp_out_corr;

        %% Correlation between neural rdm and behavioral rdm (out conflict only)
        conprdm_VECTOR = matrix2vector (behardm, "lower");
        neuralrdm_VECTOR = matrix2vector (neuralrdm, "lower");
        facerdm_VECTOR = matrix2vector (facerdm, "lower");
        %[neuralbeha_corr, neuralbeha_corr_p]  = corr(neuralrdm_VECTOR', conprdm_VECTOR', 'rows','complete', 'type','Spearman');
        [neuralbeha_corr, neuralbeha_corr_p]  = partialcorr(neuralrdm_VECTOR', conprdm_VECTOR', facerdm_VECTOR', 'rows','complete', 'type','Spearman');
        allsubject_neuralbeha_R (nSub, nBin) = neuralbeha_corr;
        
        facerdm_allsub (:,:,nBin,nSub) = facerdm;
    end
...
contrast_names = {'neural_conp_all', 'neural_conp_in', 'neural_conp_out', 'neural_beha', 'in_out'};
...
for iContrast = 1 : length (contrast_names)
    ...
    if ~strcmp (contrast_names (iContrast), 'in_out')
    ...
    elseif strcmp (contrast_names (iContrast), 'in_out')
        
        DiffR = all_R{2} - all_R{3};
        [H, P, ~, STATS] = ttest (DiffR, 0,'alpha', p_first); % test whether 
        Results_Stats(iContrast).H = H;
        Results_Stats(iContrast).P = P;
        Results_Stats(iContrast).STATS = STATS;
        Results_Stats(iContrast).conditions = contrast_names (iContrast);
    ...
    if strcmp (perm_test, 'yes')
        ...
        gate_cmp = 0;
        if strcmp (contrast_names (iContrast), 'neural_beha') 
            gate_cmp = 1;
        end
        ...
        if ~strcmp (contrast_names (iContrast), 'in_out')
            ...
            for r = 1:rand
                ...
                % make neural rdm nan
                shuffledneurdm = neurdm(randseq, randseq, :, :);
                shuffledneurdm( isnan (conp_RDM{iContrast})) = nan;
                ...
                shuffledfacerdm_reshape = reshape (facerdm_allsub, size(facerdm_allsub,1)*size(facerdm_allsub,2), size(facerdm_allsub,3), size(facerdm_allsub,4));
                ...
                for iwBin = 1:nBins
                    for iwSub = 1:nSubs
                        ...
                        r_perm (iwSub, iwBin) = partialcorr(shuffledneurdm_reshape(:, iwBin, iwSub), shuffledconrdm_reshape(:, iwBin, iwSub),shuffledfacerdm_reshape (:, iwBin, iwSub), ...
        ...
        elseif strcmp (contrast_names (iContrast), 'in_out')
            ...
            for r = 1:rand
                ...
                randsel = randi([1,2], nSubs, nBins);  % if x (in randsel) = 1 then dat1-dat2; = 2 then dat2-dat1.
                randsel1 = (randsel == 1);
                randsel2 = (randsel == 2);
        ...
        %p_threshold=p_threshold/2; % two sided test
        ...
        title(strcat(Results_Stats(iContrast).session ,'-', strrep (char(contrast_names(iContrast)), '_', '-')));
        saveas(gcf,strcat(savName,strcat(Results_Stats(iContrast).session, '_', string(Results_Stats(iContrast).conditions), '.png')));
        pause (0.5);
        clf;
        pause (0.5);
        close all;
...
for nSub = 1: nSubs
    ...
    for nBin = 1: nBins
        ...
        behardm     = RDM_post.allsubject_behardm_allbins (:, :, nBin, nSub);
        conprdm_all = RDM_post.allsubject_coceprdm_conflict_allbins (:, :, nBin, nSub);
        conprdm_in  = RDM_post.allsubject_coceprdm_inconflict_allbins (:, :, nBin, nSub);
        conprdm_out = RDM_post.allsubject_coceprdm_outconflict_allbins (:, :, nBin, nSub);
...
for iContrast = 1 : length (contrast_names)
    ...
    if ~strcmp (contrast_names (iContrast), 'in_out')
        ...
        Results_Stats(iContrast).STATS = STATS;
        Results_Stats(iContrast).conditions = contrast_names (iContrast);
        Results_Stats(iContrast).session    = "postlearning";
...
cd (path_out);
save(strcat(savName, Results_Post_Stats(iContrast).session, '.mat'),...
    'Results_Post_Stats','facerdm');
clear behardm neurdm inrdm outrdm allrdm conprdm_all conprdm_in conprdm_out neuralrdm Results_Stats;
