...
path_in  = data_folder;
savName = strcat ('Results_RSA_concepImplicit_SR',num2str(sr),'WIN',num2str(win),'SLIDE',num2str(slide), '_', ztrans, '_allrand'); 
...
path_diff = fullfile(saveLocation, 'Stats',strcat('RSA_concepImplicit_SR',num2str(sr),'WIN',num2str(win),'SLIDE',num2str(slide),'_', ztrans, '_allrand'));
if ~exist (path_diff)
    mkdir(path_diff)
end
path_out = path_diff;
...
for nSub = 1: nSubs
    ...
    for nBin = 1: nBins
        ...
        conprdm_VECTOR(isnan(conprdm_VECTOR)) = [];
        ...
        neuralrdm_VECTOR(isnan(conprdm_VECTOR)) = [];
        conprdm_VECTOR(isnan(conprdm_VECTOR)) = [];
        [neuralbeha_corr, neuralbeha_corr_p]  = corr(neuralrdm_VECTOR', conprdm_VECTOR', 'type','Spearman');
...
save(strcat(savName, strcat(Results_Pre_Stats(iContrast).session, '.mat'),...
    'Results_Pre_Stats', 'allsubject_neuralrdm_allbin_Pre_AVG'));
...
cd (path_out);
save(strcat(savName, strcat(Results_Stats(iContrast).session, '.mat'),...
    'Results_Post_Stats', 'allsubject_neuralrdm_allbin_Post_AVG'));
clear behardm neurdm inrdm outrdm allrdm conprdm_all conprdm_in conprdm_out neuralrdm Results_Stats;
