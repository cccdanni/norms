...
function RSA_timeresolved_stim70c3w1l(subs, work_dir) 
...
if nargin ==0
    ...
    badsubs   = [1214, 1215, 1238]; % 1201 - 1204: Local, 1229, 1238, 1239,1249 (reject epochs > 100)
...
path_in  = data_folder;
savName = strcat ('/', 'Results_RSAc3w1l_', task ,'_SR',num2str(sr),'WIN',num2str(win),'SLIDE',num2str(slide), '_', ztrans, '_'); 
path_out = fullfile(saveLocation, 'Results',strcat('RSAc3w1l_', task, '_SR',num2str(sr),'WIN',num2str(win),'SLIDE',num2str(slide),'_', ztrans));
mkdir(path_out)
...
allsubject_behardm_allbins = zeros (n_stim, n_stim, n_bins, numel(subs));
allsubject_coceprdm_conflict_allbins = zeros (n_stim, n_stim, n_bins, numel(subs));
allsubject_coceprdm_inconflict_allbins = zeros (n_stim, n_stim, n_bins, numel(subs));
allsubject_coceprdm_outconflict_allbins = zeros (n_stim, n_stim, n_bins, numel(subs));
...
for sub_nr = 1:numel(subs)
    ...
    conceptual_rdm_conflict = ones (n_cond*10, n_cond*10);
    for i = 1: n_cond*10
        for j = 1: n_cond*10
            if j>=i 
            ...
            elseif i<=60
                conceptual_rdm_conflict (i, j ) = nan;
            elseif (ismember(j, [1:10, 31:40])) & (ismember(i, [61:70]))
                conceptual_rdm_conflict (i, j ) = 0;
            elseif (ismember(j, [11:20, 41:50])) & (ismember(i, [61:70]))
                conceptual_rdm_conflict (i, j ) = 2;
            elseif (ceil(i/10) == ceil(j/10))
                conceptual_rdm_conflict (i, j ) = nan;
            end
        end
    end
    % conceptual rdm 1: both ingroup / outgroup higher is similar to morphed faces (dissimilar = 0), 
    % ingroup / outgroup lower is dissimilar to morphed faces (dissimilar = 2)
    % others is less dissimilar to morphed faces (dissimilar = 1)
    % only consider the dissimilarity between experimental faces and
    % morphed faces

    conceptual_rdm_inconflict = ones (n_cond*10, n_cond*10);
    for i = 1: n_cond*10
        for j = 1: n_cond*10
            if j>=i 
            ...
            elseif i<=60
                conceptual_rdm_inconflict (i, j ) = nan;
            elseif (ismember(j, [1:10])) & (ismember(i, [61:70]))
                conceptual_rdm_inconflict (i, j ) = 0;
            elseif (ismember(j, [11:20])) & (ismember(i, [61:70]))
                conceptual_rdm_inconflict (i, j ) = 2;            
            elseif (ceil(i/10) == ceil(j/10))
                conceptual_rdm_inconflict (i, j ) = nan; % same category is nan
            end
        end
    end
    % conceptual rdm 2: only ingroup higher is similar to morphed faces
    % ingroup lower is dissimilar to morphed faces (dissimilar = 2)
    % others is less dissimilar to morphed faces (dissimilar = 1)

    conceptual_rdm_outconflict = ones (n_cond*10, n_cond*10);
    for i = 1: n_cond*10
        for j = 1: n_cond*10
            if j>=i 
            ...
            elseif i<=60
                conceptual_rdm_outconflict (i, j ) = nan;
            elseif (ismember(j, [31:40])) & (ismember(i, [61:70]))
                conceptual_rdm_outconflict (i, j ) = 0;
            elseif (ismember(j, [41:50])) & (ismember(i, [61:70]))
                conceptual_rdm_outconflict (i, j ) = 2;               
            elseif (ceil(i/10) == ceil(j/10))
                conceptual_rdm_outconflict (i, j ) = nan; % same category is nan
            end
        end
    end
    % conceptual rdm 3: only outgroup higher is similar to morphed faces
    % outgroup lower is dissimilar to morphed faces (dissimilar = 2)
    % others is less dissimilar to morphed faces (dissimilar = 1)
    
    behavior_rdm = nan (n_cond*10, n_cond*10);
    thisbeha = behadata(behadata(:,3) == subs(sub_nr),:);
    for i = 1: n_cond*10
        for j = 1: n_cond*10
            if j>=i 
                behavior_rdm (i, j) = nan;
            elseif (ceil(i/10) == ceil (j/10))
                behavior_rdm (i, j) = nan;
            else
...
if strcmp (stat_test, 'yes')
    ...
    for nSub = 1: size (allsubject_neuralrdm_allbins, 4)
        ...
        for nBin = 1: size (allsubject_neuralrdm_allbins, 3)
            ...
            for nCond = 1: 6
                allsubject_neuralrdm_allbin_AVG (nSub, nCond, nBin) = mean (neuralrdm(((nCond-1)*10 +1):nCond*10, 61:70) , 'all');
            end
