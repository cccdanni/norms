...
task = {'preLearningImplicit', 'postLearningImplicit','postMinuspre'};
...
% F-test conditions
ConditionName = {'ingroup', 'outgroup'};
ConditionBin = [1:3; 4:6];
...
for iTask = 1:length(task)
    ...
    for ibin = 1:length(bindescr) % loop for each condition
        ...
        for iROI = 1:length(ROIs) % loop for each ROI
            ...
            avg = avg_all;
            zeroavg = avg;
            for k = 1:length(zeroavg)
                zeroavg{k}.powspctrm = ones(size(zeroavg{k}.powspctrm, 1), size(zeroavg{k}.powspctrm, 2), size(zeroavg{k}.powspctrm, 3));
            end

            cfg               = [];
            cfg.channel       = COI;
            cfg.latency       = [-0.200 0.996];
            cfg.frequency     = 'all';
            cfg.avgoverchan   = 'yes';
            cfg.avgovertime   = 'no';
            cfg.avgoverfreq   = 'no';
            ...
            stat = ft_freqstatistics(cfg, avg{:}, zeroavg{:});

            cfg = [];
            cfg.channel = COI;
            cfg.latency = [-0.200 0.996];
            freq1_grandavg = ft_freqgrandaverage(cfg, avg{:});
            freq2_grandavg = ft_freqgrandaverage(cfg, zeroavg{:});
            ...
            close all;
            save (strcat( curTask,'-', curbin, '-', curROI, '-onesample.mat'), 'stat', 'curROI', 'COI', 'curbin')
