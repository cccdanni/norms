...
function SVM_ECOC_ERP_Decoding(subs, work_dir) % DC: revised here
...
saveLocation = fullfile (pwd, 'Decoding', 'Results');
...
svmECOC.nBins = 3; % # of stimulus bins % DC: higher, lower vs. consistent

svmECOC.nIter = 10; % # of iterations

svmECOC.nBlocks = 15; % # of blocks for cross-validation
...
for s = 1:nSubs %decoding is performed within each subject independently
    ...
    if condition == "ingroup"
        binorgEEG.binwise_data = binorgEEG.binwise_data([1:3]);
        binorgEEG.n_trials_this_bin = binorgEEG.n_trials_this_bin([1:3]);
    elseif condition == "outgroup"
        binorgEEG.binwise_data = binorgEEG.binwise_data([4:6]);
        binorgEEG.n_trials_this_bin = binorgEEG.n_trials_this_bin([4:6]);
    end % DC: different conditions
